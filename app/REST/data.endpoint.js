...
const dataEndpoint = (router) => {
    ...
    router.get('/api/posts/:id', async (request, response, next) => {
        ...
        if (post) {
            ...
            response.status(404).send({ error: `Post with ID ${postId} not found` });
        }
    });
    ...
    router.put('/api/post/:id', async (request, response, next) => {
        ...
        if (index !== -1) {
            ...
            response.status(200).send({post: posts[index]});
        } else {
            response.status(404).send({message: `Post with ID ${postId} not found`});
        }
    });
};
